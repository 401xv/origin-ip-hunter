...
def found(text):    print(f"{C.GREEN}[+]{C.RESET} {C.GREEN}{C.BOLD}{text}{C.RESET}")
...
def http_get(url: str, host_header: str = None, timeout: int = 8) -> requests.Response | None:
...
class ResultStore:
    ...
    def add_candidate(self, ip: str, source: str, note: str = ""):
        if ip not in self.candidates:
            self.candidates[ip] = {"sources": [], "verified": False, "org": "", "notes": []}
    ...
    def add_finding(self, category: str, detail: str):
        self.findings.append({"category": category, "detail": detail, "time": str(datetime.now())})
    ...
    def report(self, output_file=None):
        ...
        lines.append(f"  Elapsed: {(datetime.now() - self.start_time).seconds}s")
        lines.append(f"{'═'*65}")

        lines.append(f"\n[CANDIDATE ORIGIN IPs — {len(self.candidates)} found]")
        if self.candidates:
            for ip, data in self.candidates.items():
                cf = "⚠ CLOUDFLARE" if is_cloudflare(ip) else "★ POSSIBLE ORIGIN"
                ...
                for h in LEAK_HEADERS:
                    ...
                    if val:
                        ...
                        for ip in ips:
                            ...
                            store.add_candidate(ip, "Config-Header", f"{h}: {val}")
                            store.add_finding("Config Leak", f"Header {h}={val} at {url}")

                # Check body for IPs
                body_ips = extract_ips(r.text[:50000])
                for ip in body_ips:
                    step(f"IP in body at {url}: {ip}")
                    store.add_candidate(ip, "Config-Body", f"Found in body of {url}")
                    store.add_finding("Config Leak", f"IP {ip} in body at {url}")

                # Check for server errors revealing info
                if r.status_code in [500, 503, 400]:
                    info(f"Error page at {url} → status {r.status_code} — inspect manually")
                    store.add_finding("Config Leak", f"Error page {r.status_code} at {url}")
        ...
        for ns_rdata in ns_answers:
            ...
            for ns_ip in ns_ips:
                try:
                    ...
                    store.add_finding("DNS Leak", f"ZONE TRANSFER SUCCESS: {ns}")
                    for name, node in z.nodes.items():
                        ...
                        for rdataset in rdatasets:
                            for rdata in rdataset:
                                ...
                                for ip in ips:
                                    store.add_candidate(ip, "DNS-AXFR", f"Zone transfer record: {name}")
                except Exception:
...
def module_historical(domain: str, store: ResultStore):
        ...
        if r and r.status_code == 200:
            ...
            for ip in ips:
                if not is_cloudflare(ip):
                    ...
                    store.add_candidate(ip, "Historical-SecurityTrails", "From SecurityTrails history")
                    store.add_finding("Historical", f"SecurityTrails historical IP: {ip}")
    except Exception as e:
        ...
        if r and r.status_code == 200:
            ...
            for ip in ips:
                if not is_cloudflare(ip):
                    ...
                    store.add_candidate(ip, "Historical-ViewDNS", "From ViewDNS history")
                    store.add_finding("Historical", f"ViewDNS historical IP: {ip}")
    except Exception as e:
        step(f"ViewDNS: {e}")
            ...
            for line in lines:
                ...
                if len(parts) == 2:
                    ...
                    for found_ip in ips_found:
                        ...
                        store.subdomains[sub] = store.subdomains.get(sub, []) + [found_ip]
                        if not is_cloudflare(found_ip):
                            found(f"Non-CF IP via HackerTarget: {found_ip} ({sub})")
                            store.add_candidate(found_ip, "Historical-HackerTarget", f"Sub: {sub}")
                            store.add_finding("Historical", f"HackerTarget {sub} → {found_ip}")
    except Exception as e:
    ...
    try:
        ...
        if r and r.status_code == 200:
            ips = extract_ips(r.text)
            subs_found = re.findall(r'([a-zA-Z0-9\-\.]+\.' + re.escape(domain) + r')', r.text)
            for ip in ips:
                if not is_cloudflare(ip):
                    step(f"RapidDNS IP: {ip}")
                    store.add_candidate(ip, "Historical-RapidDNS", "From RapidDNS")
        ...
        if r and r.status_code == 200:
            ...
            for ip in ips:
                if not is_cloudflare(ip):
                    ...
                    store.add_candidate(ip, "Historical-Wayback", "Wayback Machine CDX")
                    store.add_finding("Historical", f"Wayback Machine IP: {ip}")
    except Exception as e:
        step(f"Wayback: {e}")
                    ...
                    if ips:
                        ...
                        for ip in ips:
                            if not is_cloudflare(ip):
                                ...
                                store.add_candidate(ip, "Cert-CT", f"CT log subdomain: {sub}")
                                store.add_finding("Cert Leak", f"crt.sh {sub} → {ip}")
                        ...
                        if kind == "DNS":
                            ...
                            for ip in ips:
                                if not is_cloudflare(ip):
                                    ...
                                    store.add_candidate(ip, "Cert-SAN", f"SAN: {value}")
                                    store.add_finding("Cert Leak", f"SAN {value} → {ip}")
                        elif kind == "IP Address":
                            found(f"IP in SAN: {value}")
                            store.add_candidate(value, "Cert-SAN-IP", "Direct IP in cert SAN")
                            store.add_finding("Cert Leak", f"Direct IP in cert SAN: {value}")
    except Exception as e:
                ...
                for record in data.get("passive_dns", []):
                    ...
                    for ip in ips:
                        ...
                        if not is_cloudflare(ip):
                            ...
                            store.add_candidate(ip, "Cert-OTX", f"OTX passive DNS: {hostname}")
                            store.add_finding("Cert Leak", f"OTX {hostname} → {ip}")
            except Exception:
                pass
    except Exception as e:
        step(f"OTX: {e}")
                        ...
                        if ips:
                            ...
                            for ip in ips:
                                ...
                                if not is_cloudflare(ip):
                                    ...
                                    store.add_candidate(ip, "Sub-ThreatMiner", f"Sub: {sub}")
                                    store.add_finding("Subdomain", f"ThreatMiner {sub} → {ip}")
            except Exception:
                pass
    except Exception:
        pass
                ...
                if ips:
                    ...
                    for ip in ips:
                        ...
                        if not cf:
                            ...
                            store.add_candidate(ip, "Sub-Bruteforce", f"Subdomain: {fqdn}")
